...
> ⚠️ **Upgrade ke v2.2 direkomendasikan** — Rules v2.1 menggunakan `getUserRole()` yang membutuhkan `request.auth` untuk membaca `/test/appConfig`. Ini menyebabkan daftar Sales tidak muncul di halaman login karena `loadAppConfig()` dipanggil sebelum login (error: *Missing or insufficient permissions*).
>
...
### Cara Deploy Rules v2.2
1. Firebase Console → **Firestore Database** → tab **Rules**
2. Hapus rules lama, paste isi file `firestore.rules` versi terbaru
3. Klik **Publish** → Verifikasi via Rules Playground: read `/test/appConfig` tanpa auth → harus **ALLOW**

### Firebase Storage Rules (wajib diset manual)
```
rules_version = '2';
